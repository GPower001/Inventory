...
const Navbar = () => {
  ...
  useEffect(() => {
    // Fetch initial notifications (both low-stock and expired)
    const fetchNotifications = async () => {
      try {
        const response = await axios.get("/api/notifications");
        const notificationsData = response?.data?.data || [];
        // Only show unread notifications of type "expired" or "low-stock"
        setNotifications(
    ...
    socket.on("new-notification", (notification) => {
      // Only add if it's expired or low-stock
      if (
        notification &&
